...
public class QuadroPieceManager_Fixed : MonoBehaviour
{
    ...
    {
        // Encontra todas as peças que já estão na cena SEM modificar nada
        pecasQuadro.Clear();
    ...
    public void AtivarSistemaPecas()
    {
        ...
        // Ativa todas as peças e configura elas para serem coletíveis
    ...
    {
        // AGORA corrige tag e layer apenas quando o sistema é ativado
        if (!peca.CompareTag("Item"))
        {
            peca.tag = "Item";
            Debug.Log($"Tag da peça {peca.name} corrigida para 'Item'");
        }
        ...
        if (collectibleItem != null)
        {
            ...
            Debug.Log($"Componente CollectibleItem removido da peça {peca.name}");
        }

        // Adiciona o componente de peça coletível
        ...
        // Atribui a referência correta para este manager
        collectible.managerFixed = this;
        collectible.manager = null; // Garante que só use a versão corrigida

        // Garante que tenha um collider configurado corretamente
        Collider col = peca.GetComponent<Collider>();
        if (col != null)
        {
            col.isTrigger = false; // Não deve ser trigger para o sistema de raycast
        }
    ...
    void CompletarPuzzle()
    {
        ...
        // NOVO: Notifica o PuzzleItemManager que TODAS as peças foram coletadas
        PuzzleItemManager puzzleManager = GetComponent<PuzzleItemManager>();
        if (puzzleManager != null)
        {
            // Marca todas as peças como coletadas de uma vez
            for (int i = 1; i <= totalPecas; i++)
            {
                puzzleManager.CollectPuzzlePiece("Peça " + i);
            }
        }
    ...
    System.Collections.IEnumerator DroparChave()
    {
        ...
        if (chaveRecompensa != null && localDropChave != null)
        {
            ...
            // Configura a chave como coletível
            ...
            // Configurar a tag para ser coletível
    ...
    [ContextMenu("Forçar Ativação das Peças Agora")]
    public void ForcaAtivacaoAgora()
    {
        // Força a ativação imediata sem esperar eventos
        sistemaAtivado = false; // Reset o estado
        AtivarSistemaPecas();
        Debug.Log("Sistema de peças ativado forçadamente!");
    }
